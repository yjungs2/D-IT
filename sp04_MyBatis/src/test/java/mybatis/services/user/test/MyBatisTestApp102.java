...
public class MyBatisTestApp102 {
	...
	public static void main(String[] args) throws Exception{
		...
		System.out.println(":: 1. UserMapper10.addUser(INSERT)  ? "	+ dao.addUser(user) );
		
		//2. MyBatisUserDAOImpl10.getUser Test :: 주몽 inert 확인 
		System.out.println(":: 2. UserMapper10.getUser(SELECT)  ? \n " + dao.getUser(user.getUserId()) );

		//3. MyBatisUserDAOImpl10.uadateUser Test  
		//                                                    :: 주몽 ==> 온달 수정
		user.setUserName("온달");
		System.out.println(":: 3. UserMapper10.update(UPDATE)  ? "+dao.updateUser(user) );
		
		//4. MyBatisUserDAOImpl10.getUserList Test::Dynamic Query 확인 id=user04/name=온달 검색
		System.out.println(":: 4. UserMapper10.getUserList(SELECT)  ? ");
		TestUtil.printList( dao.getUserList(user) );
		
		//5. MyBatisUserDAOImpl10.removeUser Test
		System.out.println(":: 5. UserMapper10.Use10.removeUser (DELETE)  ? "
